...
// delete a post by id
// will delete posts that the user made themselves
    ...
    .then(profile => {
        ...
        .then(post => {
            ...
            if(post.user.toString() !== req.user.id){
                return res.status(401).json({ notauth: 'user not auth to delete'});
            }
            // delete 
...
router.post('/like/:id', passport.authenticate('jwt', {session: false}), (req, res) => {
    ...
    .then(post => {
        if(
            ...
            return res.status(400).json({liked: "user already liked dummy"});
        }
             // add userid to the likse
             post.likes.unshift({ user: req.user.id});
             post.save().then(post => res.json(post));    
     })
      .catch(err => res.status(404).json({post: 'post doesnt exijnjst'}));
}) ;
});
...
if(post.comments.filter(comment => comment.id.toString()=== req.params.comment_id).length ===0){
    return res.status(404).json({ msg: 'comment hhdoesnt exist'});
}
